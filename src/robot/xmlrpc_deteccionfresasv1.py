...
def detectar_fresas(imagen):
    # Convertir la imagen de BGR a RGB para el modelo
    RGBimg = cv2.cvtColor(imagen, cv2.COLOR_BGR2RGB)
    ...
    for detection in detections:
        if detection is not None:
            detection = rescale_boxes(detection, opt.img_size, imagen.shape[:2])
            for x1, y1, x2, y2, conf, cls_conf, cls_pred in detection:
                ...
                if int(cls_pred) == 0:  # Suponiendo que la clase 0 es "fresa"
                    ...
                    # Mostrar las coordenadas 3D y la distancia por terminal
                    print(f"Punto P{len(positions)} - Coordenadas 3D: X={x_punto}, Y={y_punto}, Z={z_punto}")
