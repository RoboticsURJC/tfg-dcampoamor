    ...
    def get_next_pose():
        ...
        if detected_points:
            ...
            if len(pose_value) == 3:  # Asegúrate de que sea de 3 elementos
                ...
                Ry = 0
                Rz = 0
                print(f"Enviando la posición detectada al robot: (x: {x}, y: {y}, z: {z}, Rx: {Rx}, Ry: {Ry}, Rz: {Rz})")
                return [x,y,z,Rx,Ry,Rz]  # Devolvemos solo x e y
            else:
                print("Error: el punto detectado no tiene suficientes coordenadas.")
                return done_pose  # Retorna una posición predeterminada en caso de error
        else:
            print("No se detectaron posiciones, esperando...")
            return done_pose  # Asegúrate de devolver un valor válido en caso de no encontrar puntos

    server = SimpleXMLRPCServer(("", 50000), allow_none=True)
    server.register_function(get_next_pose, "get_next_pose")
    print("Servidor XML-RPC escuchando en el puerto 50000...")
