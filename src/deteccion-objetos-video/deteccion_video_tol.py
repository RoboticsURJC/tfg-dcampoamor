        ...
        for detection in detections:
            if detection is not None:
                ...
                for x1, y1, x2, y2, conf, cls_conf, cls_pred in detection:
                    # Las coordenadas del rectángulo de detección son equivalentes a:
                    # (x1,y1) = (xmin, ymin) = (left, top)
                    # (x2, y2) = (xmax, ymax) = (right, bottom)
                    
                    #Calculamos el ancho y largo del rectángulo de detección
                    box_w = x2 - x1
                    box_h = y2 - y1
                    #Calculamos el punto central del rectángulo de detección
                    x_central = int(box_w/2)
                    y_central = int(box_h/2)
                    posicion = [x_central, y_central]
                    
                    #Verificamos si la posicion actual está dentro de la tolerancia de alguna posición ya detectada
                    add_pos = True
                    for pos_almacenada in l_pos:
                        distancia = ((posicion[0] - pos_almacenada[0]) ** 2 + (posicion[1] - pos_almacenada[1]) ** 2) ** 0.5
                        distancia_relativa = distancia / max(box_w, box_h) #Normalizamos respecto al tamaño del cuadro
                        if distancia_relativa <= tolerancia_porcentaje / 100:
                             add_pos = False
                             break
                   
                    #Agregamos la posicion si cumple con los criterios
                    if add_pos:
                        l_pos.append(posicion)      
                        color = [int(c) for c in colors[int(cls_pred)]]
                        #print("Se detectó {} en X1: {}, Y1: {}, X2: {}, Y2: {}".format(classes[int(cls_pred)], x1, y1, x2, y2))
                        print("El punto central del recuadro de la detección {} es X: {}, Y: {}".format(classes[int(cls_pred)], x_central, y_central)) #En píxeles
